...
l = 0.5 # percent of tree resources lost from ROI allocation
...
Differences in fungal pop sizes can be overcome if preference is 50%. 
Difference in host preference for one fungus or the other results in uneven but coexisting fungal populations. 
When fungal populations are both initially different and preference is included, the effect of the preference drives the result (see above). 
...
When the second set (either tree or tree and fungus) are started initially low, the host pools stay low but the fungus reaches the same eq size as the other fungus. 
